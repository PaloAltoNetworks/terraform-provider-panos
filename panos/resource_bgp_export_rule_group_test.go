...
func TestAccPanosBgpExportRuleGroup_basic(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy: testAccPanosBgpExportRuleGroupDestroy,
		Steps: []resource.TestStep{
			{
				Config: testAccBgpExportRuleGroupConfig(vr, m1, m2, m3, m4, m5),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckPanosBgpExportRuleGroupExists("panos_bgp_export_rule_group.top", "panos_bgp_export_rule_group.mid", "panos_bgp_export_rule_group.bot", &o1, &o2, &o3, &o4, &o5),
					testAccCheckPanosBgpExportRuleGroupAttributes(&o1, &o2, &o3, &o4, &o5, m1, m2, m3, m4, m5),
					testAccCheckPanosBgpExportRuleGroupOrdering(vr),
				),
			},
			{
				Config: testAccBgpExportRuleGroupConfig(vr, m6, m7, m8, m9, m10),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckPanosBgpExportRuleGroupExists("panos_bgp_export_rule_group.top", "panos_bgp_export_rule_group.mid", "panos_bgp_export_rule_group.bot", &o1, &o2, &o3, &o4, &o5),
					testAccCheckPanosBgpExportRuleGroupAttributes(&o1, &o2, &o3, &o4, &o5, m6, m7, m8, m9, m10),
					testAccCheckPanosBgpExportRuleGroupOrdering(vr),
				),
			},
		},
...
func testAccCheckPanosBgpExportRuleGroupExists(top, mid, bot string, o1, o2, o3, o4, o5 *exp.Entry) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		vr, _, _, topList := parseBgpExportRuleGroupId(rTop.Primary.ID)
		if len(topList) != 2 {
			return fmt.Errorf("top is not len 2")
		}
		v1, err := fw.Network.BgpExport.Get(vr, topList[0])
		if err != nil {
			return fmt.Errorf("Failed to get top0: %s", err)
		}
		*o1 = v1
		v2, err := fw.Network.BgpExport.Get(vr, topList[1])
		if err != nil {
		...
		*o2 = v2
		...
		vr, _, _, midList := parseBgpExportRuleGroupId(rMid.Primary.ID)
		if len(midList) != 1 {
			return fmt.Errorf("mid is not len 1")
		}
		v3, err := fw.Network.BgpExport.Get(vr, midList[0])
		if err != nil {
		...
		*o3 = v3
		...
		vr, _, _, botList := parseBgpExportRuleGroupId(rBot.Primary.ID)
		if len(botList) != 2 {
			return fmt.Errorf("bot is not len 2")
		}
		v4, err := fw.Network.BgpExport.Get(vr, botList[0])
		if err != nil {
			return fmt.Errorf("Failed to get bot: %s", err)
		}
		*o4 = v4
		v5, err := fw.Network.BgpExport.Get(vr, botList[1])
		if err != nil {
...
func testAccCheckPanosBgpExportRuleGroupAttributes(o1, o2, o3, o4, o5 *exp.Entry, m1, m2, m3, m4, m5 string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		if o1.Name != "mary" {
...
func testAccCheckPanosBgpExportRuleGroupOrdering(vr string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		fw := testAccProvider.Meta().(*pango.Firewall)
		p3i := -1
...
func testAccPanosBgpExportRuleGroupDestroy(s *terraform.State) error {
	fw := testAccProvider.Meta().(*pango.Firewall)

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "panos_bgp_export_rule_group" {
			continue
		}

		if rs.Primary.ID != "" {
			vr, _, _, list := parseBgpExportRuleGroupId(rs.Primary.ID)
			for _, rule := range list {
				_, err := fw.Network.BgpExport.Get(vr, rule)
				if err == nil {
...
func testAccBgpExportRuleGroupConfig(vr, m1, m2, m3, m4, m5 string) string {
	return fmt.Sprintf(`
...
resource "panos_bgp_export_rule_group" "top" {
...
resource "panos_bgp_export_rule_group" "mid" {
    virtual_router = panos_bgp.x.virtual_router
    position_keyword = "before"
    position_reference = panos_bgp_export_rule_group.bot.rule.0.name
    rule {
        name = "a"
        match_as_path_regex = %q
        action = "deny"
...
resource "panos_bgp_export_rule_group" "bot" {
    virtual_router = panos_bgp.x.virtual_router
