...
func TestAccPanosPanoramaHttpServerProfile_basic(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy: testAccPanosPanoramaHttpServerProfileDestroy,
		Steps: []resource.TestStep{
			{
				Config: testAccPanoramaHttpServerProfileConfig(tmpl, name),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckPanosPanoramaHttpServerProfileExists("panos_panorama_http_server_profile.test", &o, &serverList, &headers, &params),
					testAccCheckPanosPanoramaHttpServerProfileAttributes(&o, &serverList, &headers, &params, name),
				),
			},
		},
	})
...
func testAccCheckPanosPanoramaHttpServerProfileExists(n string, o *http.Entry, serverList *[]server.Entry, headers *map[string][]header.Entry, params *map[string][]param.Entry) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		tmpl, ts, vsys, dg, name := parsePanoramaHttpServerProfileId(rs.Primary.ID)
		v, err := pano.Device.HttpServerProfile.Get(tmpl, ts, vsys, dg, name)
		if err != nil {
...
func testAccCheckPanosPanoramaHttpServerProfileAttributes(o *http.Entry, serverList *[]server.Entry, headers *map[string][]header.Entry, params *map[string][]param.Entry, name string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
...
func testAccPanosPanoramaHttpServerProfileDestroy(s *terraform.State) error {
	pano := testAccProvider.Meta().(*pango.Panorama)

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "panos_panorama_http_server_profile" {
			continue
		}

		if rs.Primary.ID != "" {
			tmpl, ts, vsys, dg, name := parsePanoramaHttpServerProfileId(rs.Primary.ID)
			_, err := pano.Device.HttpServerProfile.Get(tmpl, ts, vsys, dg, name)
			if err == nil {
...
func testAccPanoramaHttpServerProfileConfig(tmpl, name string) string {
	return fmt.Sprintf(`
resource "panos_panorama_template" "x" {
    name = %q
    description = "for http server profile test"
}

resource "panos_panorama_http_server_profile" "test" {
